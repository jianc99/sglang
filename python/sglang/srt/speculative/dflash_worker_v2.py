...
class DFlashWorkerV2(DFlashWorker):
    ...
    def _append_target_hidden_to_draft_kv_by_loc(
        ...
        *,
        ...
        This helper is kept for spec-v2 overlap scheduling, where the caller already
        knows the exact cache locations to write. It is adapted to the new DFlashWorker
        so it can reuse fused KV materialization when available.
        """
        ...
        if mask_valid is not None:
            if mask_valid.ndim != 1:
                raise ValueError(
                    f"DFLASH mask_valid must be 1D, got shape={tuple(mask_valid.shape)}."
                )
        ...
        with torch.inference_mode():
            ...
            if mask_valid is None:
                if self._use_fused_kv_materialize and self._fused_kv_helper is not None:
                    try:
                        ...
                        # Match new-worker behavior: disable fused path on failure and retry sequentially.
                        import logging
            ...
            # Masked path: we cannot directly use the fused helper because invalid slots
            # should write zeros rather than real K/V values.
    ...
    def forward_batch_generation(
        ...
        **kwargs,
        ...
        if lm_head is None or not hasattr(lm_head, "weight") or not hasattr(
            lm_head, "shard_indices"
        ):
            raise RuntimeError(
