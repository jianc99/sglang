...
class DFlashWorker:
    ...
    def __init__(
        ...
        # Compatibility: overlap scheduling / disaggregation paths expect
        # `spec_worker.draft_worker.draft_runner` (Eagle-style). For DFLASH, the draft
        # worker is a `TpModelWorker`, so expose an alias for the draft model runner.
        ...
        # Overlap output processing expects the draft worker to expose the stride via
        # `speculative_num_draft_tokens` (Eagle-style). Keep it in sync with `block_size`.
    ...
    def _append_target_hidden_to_draft_kv(
        ...
        if int(draft_input.ctx_lens.numel()) != bs:
            raise RuntimeError(
                "DFLASH ctx_lens length mismatch: "
                f"got {int(draft_input.ctx_lens.numel())} for bs={bs}."
            )
        if int(draft_input.draft_seq_lens.numel()) != bs:
            raise RuntimeError(
                "DFLASH draft_seq_lens length mismatch: "
                f"got {int(draft_input.draft_seq_lens.numel())} for bs={bs}."
            )
        ...
        if bs == 1:
            # Single-request fast path: build exactly `total_ctx` positions and avoid boolean
            # masking / advanced indexing reshapes.
            max_ctx = int(total_ctx)
            if max_ctx <= int(self._block_pos_offsets.numel()):
            ...
            # Avoid `.item()` sync in the steady-state decode loop (ctx_lens <= block_size there).
        ...
        with torch.inference_mode():
            ...
            if int(ctx_hidden.shape[0]) != int(ctx_cache_loc.numel()):
                raise RuntimeError(
                    "DFLASH ctx_hidden length mismatch after packing. "
                    f"ctx_hidden={int(ctx_hidden.shape[0])}, ctx_cache_loc={int(ctx_cache_loc.numel())}."
                )
    ...
    def _append_target_hidden_to_draft_kv_by_loc(
        ...
        *,
        ...
        This helper avoids boolean-index "packing" (which can introduce stream sync due to
        dynamic output shapes). Callers should pass dense `cache_loc` and set invalid
        positions to a safe padded slot (typically 0). If `mask_valid` is provided, K/V
        for invalid positions are zeroed before writing.
        """
